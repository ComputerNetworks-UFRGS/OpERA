...
from ssf.algorithm      import QNoise, Chimas, sensing
from ssf.uhd.block_util import group_in_n, simple_tx
from ssf.uhd.uhd_wrapper      import UHDWrapper
from ss_utils           import Logger, Channel, ChannelModel, ChannelModeler
from ssf.uhd.architecture.ranking import *


if __name__ == '__main__':
	##
	#
	def dumpList(it, direc, ss_result, qn_result):
	...
	def build_gnuradio(gain, channel_list):
		...
		# Transmitter (cause interference)
		uhd_sink   = UHDSink('addr=143.54.83.28')
		uhd_sink.samp_rate = uhd_source.samp_rate =195512
		fake_device = ChannelModeler(
				device = UHDWrapper( device = uhd_sink, algorithm = None ),
		...
		device_sink = UHDDevice(
				...
				the_sink = fake_device, uhd_device = fake_device
			)
		...
		tb.connect( device_sink.source, tx_path, device_sink)
		tb.tx = device_sink

		# Receiver. Execute chimas algorithm
		uhd_source   = UHDSource('addr=143.54.83.29')
		tb.rx = ranking_arch(device = uhd_source,
	...
	## Build QNoise class
	def build_qnoise(n_weight, n_data, h_weight, h_data):
	...
	for conf in confs:
		...
		qnoise = build_qnoise(n_weight = conf['n_weight'], n_data = conf['n_data'], h_weight = conf['h_weight'], h_data = conf['h_data'])
		...
		for it in  xrange(conf['epochs']):
			...
			for ch in channel_list:
				...
				ss_result.append( [ the_channel.channel, data[-1][0], tup_arr ] )

			ln_result = qnoise.evaluate( ss_result )
			channel_list = reorder( channel_list, ln_result)
			dumpList(it, d_, ss_result, ln_result)
