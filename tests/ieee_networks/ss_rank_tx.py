...
from ss_utils                 import Channel, TopBlock, Logger, ChannelModeler
...
def buildUPBlock(options, channel_list):
	# TOP BLOCK
...
def transmitter_loop(tb, channel_list, channel, options):
	...
	Logger.register('transmitter', ['channel', 'status', 'pkt', 'elapsed'])
	...
	while time.time() < (start_t + options.duration):
		...
		if not options.tx_only:
			# Sense
			t_now = time.clock();
			t_namespace.status =  tb.rx.senseChannel( channel_list[ channel ], options.sensing_duration )
			print "Sensed for %f seconds" % (time.clock() - t_now)
			...
			if t_namespace.status > 0.000000005 :
				...
				Logger.append('transmitter', 'elapsed',  t_elapsed )
...
def main( options ):
	...
	tb = buildUPBlock(options, channel_list) if  options.interferer else buildUSBlock(options)
