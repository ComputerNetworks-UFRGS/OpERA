...
class BayesLearningThreshold(AbstractAlgorithm):
    ...
    def __init__(self,
            ...
            delta_th,
            k):
        Logger.register('bayes_decision', ['threshold', 'energy', 'hiphotesis', 'real_state', 'feedback', 'risk', 'pd', 'pf', 'pm', 'db'])
    ...
    def update_th_counter(self, signal):
        ...
        self.update_th(th, self._hiphotesis)

        if th_p in self._r:
            self.update_th(th_p, self._hiphotesis)

        if th_n in self._r:
            self.update_th(th_n, self._hiphotesis)
    ...
    def get_min_risk(self, bayes_hip):
    ...
    def decision(self, signal):
        # hiphotetis
        bayes_hip = self.bayesian_hiphotesis(self._th, signal)
        self._hiphotesis = bayes_hip
        ...
        bayes_R, self._th = self.get_min_risk(bayes_hip)

        # Save all data
        Logger.append('bayes_decision', 'threshold',  self._th)
        #Logger.append('bayes_decision', 'hiphotesis', 1 if bayes_hip > 0 else 0 )
