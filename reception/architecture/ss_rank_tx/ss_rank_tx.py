...
from utils.sensing                 import Channel, TopBlock, Logger
...
def build_up_block(options):
	# TOP BLOCK
	tb = TopBlock(name = 'RX BLOCK')

	def callbackRx(ok, payload):
		global t_rcv, t_cor
...
def transmitter_loop(tb, channel_list, channel, options):
	...
	while time.time() < (start_t + options.duration):
		...
		if not options.tx_only:
			# Sense
			status =  tb.rx.sense_channel(channel_list[ channel ], 1)
...
def main( options ):
	...
	tb = build_up_block(options) if  options.interferer else build_us_block(options)
