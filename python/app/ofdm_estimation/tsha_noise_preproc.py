...
class Correlator(gr.sync_block):
    ...
    def dump(self, options):
        for tup, blk in self.bulk_corr.items():
            print "##### Saving ", tup
            with open("./correlations/%d_%d_%d_%d_%d_%d_noise.gz" % (options.fft_length, options.cp_length, options.ebn0, options.it, THE_TUPLE[0], THE_TUPLE[1]), "w+") as fd:
...
if __name__ == "__main__":
    ...
    for tup in Correlator().ALL_PAIRS:
        ...
        #d =  "always_occupied_noise/ebn0_%d/" % options.ebn0
