...
class Correlator(gr.sync_block):
    ...
    def dump(self, options):
        for tup, blk in self.bulk_corr.items():
            print "##### Saving ", tup
            with open("./correlations/%d_%d_%s_%s_%d.gz" % (options.fft_length, options.cp_length, options.att, options.urban, options.it), "w+") as fd:
                import yaml
...
if __name__ == "__main__":
    ...
    URBAN      = ["Brazil_A", "Brazil_B", "Brazil_C"]
    ATTENUATIONS = ["00", "-05", "-10", "-15", "-20" ]

    CONVERT_FILES = {}
    for urban in URBAN:
        CONVERT_FILES[urban] = {}
        for att in ATTENUATIONS:
            CONVERT_FILES[urban][att] = 0
    ...
    the_files = [
            "/Users/maiconkist/Music/realDTVsignal/coleta_part1.tar.gz",
            "/Users/maiconkist/Music/realDTVsignal/coleta_part2.tar.gz",
            "/Users/maiconkist/Music/realDTVsignal/coleta_part3.tar.gz",
            "/Users/maiconkist/Music/realDTVsignal/coleta_part4.tar.gz"
            ]

    for tf in the_files:
        tar_file =  tarfile.open(tf, "r")
        ...
        for _f in tar_file.getmembers():
            ...
            if not _f in CONVERT_FILES["parsed"]:
                ...
                if not (att == None or urban == None) and CONVERT_FILES[urban][att] < 4:
                    ...
                    for tup in Correlator().ALL_PAIRS:
                        ...
                        options.att = att
                        options.urban = urban
                        options.it = CONVERT_FILES[urban][att]
                        options.threshold = 100
                        options.fft_length  = THE_TUPLE[0]
                        options.cp_length   = THE_TUPLE[1]

                        log = "#### FILE: %s - Att %s, urbaning: %s -- CORR FFT %d CP %d [%d]" % (options.file, att, urban, THE_TUPLE[0], THE_TUPLE[1], options.it)
                        print log
                    ...
                    CONVERT_FILES[urban][att] += 1

                    with open("parsed_files_tv.log", "w+") as fd:
