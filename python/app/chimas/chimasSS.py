...
#::TODO:: atualizar os imports, que estao bastante desatualizados
# Project imports
...
def build_gnuradio(gain, channel_list):
    ...
    # ::TODO:: atualizar esse radio device, que eh da versao antiga
    ...
    rx_path = RankingArch(device=device_source,
                          detector=SimpleRankingDetector(fft_size=1024,
                                                         mavg_size=5,
                                                         ed_threshold=0.0001),
                          max_items_group=max_items_group)

    ##tb.add_arch(rx_path, device_source, 'rx')
    ...
    src = blocks.vector_source_f(map(int, np.random.randint(0, 100, 1000)), True)
    sink = uhd_sink
    uhd_device = uhd_sink

    radio_sink = RadioDevice()

    #::TODO:: conferir o que pode ser arch aqui (tx_path ou radio_proxy?????)
    radio_sink.add_arch(source=src, arch=None, sink=sink, uhd_device=uhd_device, name="sink")
...
# ::TODO:: isso deveria ser no main???
confs = [
        #{'gain': 1, 'n_weight': 0.1 , 'n_data': (0.5, 3, [0.2, 0.35, 0.45] ), 'h_weight': 0.9, 'h_data': (0.5, 3, [0.2, 0.35, 0.45] ), 'epochs': 100}, # 0.5, 0.5
        {'gain': 1, 'n_weight': 0.2, 'n_data': (0.5, 3, [0.2, 0.35, 0.45]), 'h_weight': 0.8,
         'h_data': (0.5, 3, [0.2, 0.35, 0.45]), 'epochs': 300},  # 0.2, 0.8
        {'gain': 1, 'n_weight': 0.5, 'n_data': (0.5, 3, [0.2, 0.35, 0.45]), 'h_weight': 0.5,
         'h_data': (0.5, 3, [0.2, 0.35, 0.45]), 'epochs': 300},  # 0.5, 0.5
        {'gain': 1, 'n_weight': 0.8, 'n_data': (0.5, 3, [0.2, 0.35, 0.45]), 'h_weight': 0.2,
         'h_data': (0.5, 3, [0.2, 0.35, 0.45]), 'epochs': 300},  # 0.8, 0.2
        #{'gain': 1, 'n_weight': 0.9 , 'n_data': (0.5, 3, [0.2, 0.35, 0.45] ), 'h_weight': 0.1, 'h_data': (0.5, 3, [0.2, 0.35, 0.45] ), 'epochs': 100}, # 0.8, 0.2
]

## TODO:: deveria ter um main aqui??
...
for conf in confs:
    ...
    qnoise = build_q_noise(n_weight=conf['n_weight'], n_data=conf['n_data'], h_weight=conf['h_weight'],
                           h_data=conf['h_data'])
