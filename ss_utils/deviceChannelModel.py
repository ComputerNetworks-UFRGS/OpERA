...
class ChannelThread(threading.Thread):
	...
	def __init__(self, channel_modeler, device, the_channel, dist_callback):
		...
		self._active = False
		self._status = random.randrange(0, 2, 1)
...
class ChannelModeler(object):
	...
	def _disableThread(self, channel_thread):
		if channel_thread:
	...
	##
	# @param channel_thread
	def _enableThread(self, channel_thread):
		if channel_thread:
	...
	def center_freq(self, channel):
		...
		self._disableThread( self._curThread )                         # disable current thread
		self._enableThread( the_thread )      # activate it
	...
	def __setattr__(self, var, val):
		...
		if hasattr(self, var):
			...
			elif var == '_disableThread':
				self._disableThread(val)
			elif var == '_enableThread':
				self._enableThread(val)
		else:
