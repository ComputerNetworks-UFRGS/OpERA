...
from ss_utils.top_block       import TopBlock
from ssf.uhd.block_util       import group_in_n
from ssf.algorithm            import sensing
from ssf.uhd.detectors.energy import energy_calculator
...
class qlearning_worker(gr.sync_block):
...
class simple_ranking_detector(gr.hier_block2):
	...
	def __init__(self,
		...
		self.ql  = qlearning_worker(fft_size, algorithm = sensing.EnergyAlgorithm( ed_threshold ))
		...
		self.connect(self.c2mag_0, energy_calculator(fft_size,  None),  self.mavg, energy_calculator(1,  sensing.EnergyAlgorithm(ed_threshold)), (self, 2))


## Ranking architecture.
# Construct the ranking architecture.
class ranking_arch( UHDSSArch ):
	...
	# Reimplement from UHDSSArch::senseChannel
	# @param the_channel Channel to be sensed.
	# @param sensing_time Sensing duration on channel.
	def senseChannel(self, the_channel, sensing_time):

		data = self._getSensingData( the_channel, sensing_time )
...
class RankingRunner( TopBlock ):
	...
	## CTOR
	# @param device A UHDDevice instance.
	...
	def __init__(self,
		...
		# processing blocks
		self._device = device
		self.rx = ranking_arch(device = device, detector = detector, group_vlen = group_vlen)
