...
class QChannel:
	...
	#
	def getQVal(self):
		return self._hw * self._historic.getQVal() +  self._nw * self._noise.getQVal()

	##
	# @param final_decision Final decision regarding channel occupancy
	# @param usensing Array of tuples (dec, rssi)
	def calcQVal(self, final_decision, usensing):

		# calculate RSSI only if channel if idle
		if final_decision == 0:
			self._noise.calcQVal( usensing )

		self._historic.calcQVal( usensing )
...
class QNoise:
	...
	def evaluate(self, matrix):
		...
		for channel in (range(0, len(matrix))):
			...
			qvalue_list.append( self._channel[ch].getQVal() )
